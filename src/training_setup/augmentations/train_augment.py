...
def get_train_transforms():
    """
    Labeled training transforms. Expects keys: ["t2w", "adc", "hbv", "seg"].
    """
    ...
    return Compose([
        ...
        # Spacing: images bilinear, labels nearest
        Spacingd(keys=keys, pixdim=(1.0, 1.0, 1.0),
                 mode=("bilinear", "bilinear", "bilinear", "nearest")),
        # Intensity scale (tweak a_min/a_max if your ranges differ)
        ScaleIntensityRanged(
            keys=img_keys, a_min=0, a_max=1000, b_min=0.0, b_max=1.0, clip=True
        ),
        RandGaussianNoised(keys=img_keys, prob=0.15, mean=0.0, std=0.05),
        RandGaussianSmoothd(keys=img_keys, prob=0.2, sigma_x=(0.5, 1.0)),
        ...
        RandZoomd(
            keys=keys, prob=0.2, min_zoom=0.9, max_zoom=1.1,
            mode=("bilinear", "bilinear", "bilinear", "nearest")
        ),
